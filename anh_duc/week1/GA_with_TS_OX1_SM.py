...
class GeneticAlgorithm(object):
    ...
    def crossover_OX1(self, par1, par2):
        ...
        if (f < self.crossover_rate):
            cp1 = random.randint(0, self.gen_size - 1)
            cp2 = random.randint(0, self.gen_size - 1)
            while (cp1 >= cp2):
                cp1 = random.randint(0, self.gen_size - 1)
                cp2 = random.randint(0, self.gen_size - 1)
    ...
    def swap_mutation(self, child):
        ...
        if (f < self.mutation_rate):
            ...
            while (id_1 == id_2):
                id_1, id_2 = random.randint(0, self.gen_size - 1) , random.randint(0, self.gen_size - 1)
            child[id_1], child[id_2] = child[id_2], child[id_1]
        return child
    ...
    def creat_child(self, parents):
        ...
        for k in range(0, self.pop_size):
            child[k] = self.swap_mutation(child[k])
        return child

    def draw_chart(self, Epochs, TS_OX1_SM, mean_time, score):
        plt.plot(Epochs, TS_OX1_SM, 'r-')
    ...
    def implement_with_SM(self, pop):
        sta_time = []
        best = []
        
        Epochs = np.arange(1, self.epochs + 1, dtype=int)
        Epochs = np.reshape(Epochs, (self.epochs, 1))
        TS_OX1_SM = np.zeros((self.epochs, 1), dtype=float)

        fitness = self.get_fitness(pop)
        parents = self.select_parents(pop, fitness)
        with open('D:\\Lab609\\GeneticAlgorithm\\TS_OX1_SM.csv', 'w') as csvfile:
            fieldnames = ['Epochs', 'TS_OX1_SM']
            ...
            for id in range(0, self.epochs):
                ...
                TS_OX1_SM[id] = best_score
                             
                fitness = self.get_fitness(child)
                parents = self.select_parents(child, fitness)
                
                writer.writerow({'Epochs' : Epochs[id,0], 'TS_OX1_SM' : TS_OX1_SM[id,0]})
            ...
            score = min(best)
            # draw chart
            self.draw_chart(Epochs, TS_OX1_SM, mean_time, score)
...
if __name__ == "__main__":
    ...
    GA.implement_with_SM(population)