...
class GeneticAlgorithm(object):
    ...
    # mutation with scramble_mutation
    def scramble_mutation(self, child):
        f = np.random.random_sample()
        if (f < self.mutation_rate):
            id_1, id_2 = random.randint(0, self.gen_size - 1) , random.randint(0, self.gen_size - 1) # select random index
            while (id_1 >= id_2):
                id_1, id_2 = random.randint(0, self.gen_size - 1) , random.randint(0, self.gen_size - 1)
                np.random.shuffle(child[id_1:id_2+1])
        return child
    ...
    def creat_child(self, parents):
        ...
        for k in range(0, self.pop_size):
            child[k] = self.scramble_mutation(child[k])
        return child

    def draw_chart(self, Epochs, TS_OX1_ScM, mean_time, score):
        plt.plot(Epochs, TS_OX1_ScM, 'r-')
    ...
    def implement_with_ScM(self, pop):
        sta_time = []
        best = []
    
        Epochs = np.arange(1, self.epochs + 1, dtype=int)
        Epochs = np.reshape(Epochs, (self.epochs, 1))
        TS_OX1_ScM = np.zeros((self.epochs, 1), dtype=float)

        fitness = self.get_fitness(pop)
        parents = self.select_parents(pop, fitness)
        with open('D:\\Lab609\\GeneticAlgorithm\\TS_OX1_ScM.csv', 'w') as csvfile:
            #fieldnames = ['Epochs', 'Best_score']
            fieldnames = ['Epochs','TS_OX1_ScM']
            ...
            for id in range(0, self.epochs):
                ...
                TS_OX1_ScM[id] = best_score
                             
                fitness = self.get_fitness(child)
                parents = self.select_parents(child, fitness)
                
                writer.writerow({'Epochs' : Epochs[id,0], 'TS_OX1_ScM' : TS_OX1_ScM[id,0]})
