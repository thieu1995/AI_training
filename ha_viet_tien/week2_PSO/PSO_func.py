...
class PSO:
    ...
    #person_deg, social_deg
        ...
        self.Pbest = np.copy(self.score)
        self.Pposition = np.copy(self.pop)
        self.Gbest = []
        self.Gposition = []
        self.cluster = []
        self.neighbour = []
...
class PSO_star:
    ...
    #person_deg, social_deg
        ...
        self.Pbest = np.copy(self.score)
        self.Pposition = np.copy(self.pop)
        self.Gbest = np.copy(self.score[0])
        self.Gposition = np.copy(self.pop[0])
        # self.c1 = person_deg
        # self.c2 = social_deg
    ...
    def update_Gbest(self):
        ...
        # find it in the while population
        for i in range(self.size_pop):
            if self.score[i] < self.Gbest:
                self.Gbest = np.copy(self.score[i])
                self.Gposition = np.copy(self.pop[i])
    ...
    def update_velocity(self, time, iteration, pre_w):
        ...
        for i in range(self.size_pop):
            ...
            self.velocity[i] = w * self.velocity[i] + \
                                c1*r1*(self.Pposition[i] - self.pop[i]) + \
                                c2*r2*(self.Gposition - self.pop[i])
